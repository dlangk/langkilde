...
3. **[The Frontier Will Become More Jagged](#the-jagged-frontier).** Due to the verification bottleneck, progress will be uneven across domains. This unevenness will be amplified by which problems match the inductive biases of current models.
...
**Language is lossy.** Language is the best tool we have for transmitting preferences from one mind to another, but it still sucks. It's indirect and incomplete. When a product manager writes a product brief, they are transmitting a compressed, partial representation of what they actually want. Much of what they want is [~tacit knowledge|Knowledge that a person possesses but cannot fully articulate. It's encoded in years of experience, aesthetic judgment, taste, and contextual understanding. A chef knows when a sauce is "right" without being able to specify the exact threshold. An experienced driver knows a lane merge is dangerous without computing time-to-collision.] that they couldn't write down even if they tried.

**People change based on what they are shown.** Learning is a [~coupled problem|A learning scenario where the learner's actions change the target being learned. Markets shift when you trade. Users' preferences change when you show them options. The target you learned from may no longer exist, partly because learning it changed the system.]. Often times, you don't know what you want until you see what you don't want. A designer shows a client three mockups: the client rejects all three but now has a much clearer sense of what they're after. This sense *did not exist* before seeing the mockups. Their preference is literally created by the process of you trying to learn it.
...
Many of the problems that matter most (social dynamics, markets, organizations and geopolitics) are [~non-stationary|A system in which the generating process changes over time. What was true yesterday may not be true today, not because you learned it wrong, but because the world changed. Non-stationary problems reward speed and adaptability over depth.] The target is moving. Past observations become irrelevant as the world drifts away from the process that generated them.
...
A common mistake when thinking about AI is to assume that intelligence makes search easy. It doesn't. Some problems are intrinsically hard to search. For example, I don't expect superintelligent AI to crack AES encryption. The search space is 2^128 regardless of intelligence. There are physical limits to what we can expect for large search spaces. [^quantum]
...
Formal verification languages like Coq and Lean are underused not because humans are bad at them, but because [~program search|The computational problem of finding a program that satisfies a given specification. The search space grows exponentially with program length, making exhaustive search intractable for all but trivial programs.] is hard. More intelligence doesn't change the size of the search space. It may let you move through it faster, and sometimes deep analysis can help you shrink it. But if it's large, then there isn't much you can do about it.
...
**Now, the obvious objection: compute costs are falling fast.** Yes, of course. They always have been. GPUs are getting cheaper, model architectures get more efficient and inference chips are created. Won't the boundary keep shifting until everything is worth automating? This is essentially testing if [~Jevons paradox|The observation that increasing efficiency in the use of a resource often increases total consumption of that resource rather than decreasing it. Cheaper computing doesn't reduce computing spending; it makes more problems worth throwing computing at.] holds for intelligence.
...
25. OpenAI. (2025). *Sycophancy in GPT-4o*. [openai.com](https://openai.com/index/sycophancy-in-gpt-4o/)
26. Yun, C., Bhojanapalli, S., Rawat, A. S., Reddi, S. J., & Kumar, S. (2020). *Are Transformers universal approximators of sequence-to-sequence functions?* ICLR 2020. [arXiv:1912.10077](https://arxiv.org/abs/1912.10077)
